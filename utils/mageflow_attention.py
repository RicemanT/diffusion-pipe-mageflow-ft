...
def validate_attention_backend(backend, pipeline_stages=1):
    if backend not in ('sdpa', 'flash_attn_2', 'flash_attn_3'):
        raise ValueError('MageFlow attention_backend must be sdpa, flash_attn_2, or flash_attn_3')
    if backend != 'sdpa':
        ...
        if pipeline_stages != 1:
            raise ValueError('MageFlow packed attention currently requires pipeline_stages=1')
        try:
...
def packed_attention(q, k, v, indices, cu_seqlens, backend, deterministic=False):
    ...
    return _packed_attention(q, k, v, indices, cu_seqlens, fn, deterministic)


def _packed_attention(q, k, v, indices, cu_seqlens, fn, deterministic=False):
    """Layout conversion shared by real kernels and numerical reference tests."""
    batch, heads, length, dim = q.shape

    def pack(x):
        return x.transpose(1, 2).reshape(batch * length, heads, dim).index_select(0, indices)

    # Padded length is a safe upper bound: avoids a GPU->CPU max().item().
    out = fn(pack(q), pack(k), pack(v), cu_seqlens, cu_seqlens,
             length, length, causal=False, deterministic=deterministic)
